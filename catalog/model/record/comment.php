...
public function getCommentByMark ($data, $mark)
{
    ...
    if ($mark == 'product') {
     ...
     $sql_product_join_order = "LEFT JOIN `" . DB_PREFIX . "order_product` op ON (op.order_id = o.order_id AND o.customer_id <> 0 AND o.order_status_id IN ".$sql_status." AND op.product_id = '".$data['record_id']."')";

     $sql_product_join = "LEFT JOIN `" . DB_PREFIX . "product` p ON (p.product_id = '".$data['record_id']."')";
