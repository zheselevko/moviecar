...
class GdWatermarkLib
{
	...
	public function createWatermark($image_path, $right_margin, $bottom_margin, $that)
	{
		...
		$this->format      		 = $this->parentInstance->getFormat();

		//$this->parentInstance->determineFormat();
		...
		imagecopy($this->newImage,
				  $stamp,
				  imagesx($this->newImage) - $sx - $right_margin,
				  imagesy($this->newImage) - $sy - $bottom_margin,
				  0,
				  0,
				  imagesx($stamp),
				  imagesy($stamp));
